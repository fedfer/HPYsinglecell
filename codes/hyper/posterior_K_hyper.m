...
function [M_Tavoli, M_l_star, M_parametri, Dati_star, k_popolazioni]=posterior_K_hyper(Dati,M0,V0,J,n_init,T,t_burn)
...
for ii=1:T
    ...
    theta_0 = MH_theta_hyper(theta_0,sigma_0,sum(L),k,theta_h);

	[sigma_0] = MH_sigma_hyper(sigma_0,theta_0,l_star_franchise,a0,b0);
    
    [a0, b0] = MH_a0b0(a0,b0,sigma,J);
    
    [theta_h] = MH_theta_h(theta_h,theta,J);
    
    if ii>t_burn
        for j=1:J
            M_Tavoli{j}(ii-t_burn,:)=Tavoli{j};
        end
        M_parametri(ii-t_burn,:)=[theta sigma theta_0 sigma_0 theta_h, a0, b0];
    end
end
