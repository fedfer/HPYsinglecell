...
data_set=cell(1,2);
data_set{1}='clementina_flower.txt';
data_set{2}='clementina_root.txt';
n=zeros(1,2);
k=zeros(1,2);
...
cluster_flower=cluster{1};
cluster_root=cluster{2};
n1=n(1);
n2=n(2);
k1=k(1);
k2=k(2);
...
for i=1:k1
    indice_corrente=[];
    for j=1:k2
        if length(geni_flower{i})~=length(geni_root{j})
            continue;
        end
        if geni_root{j}==geni_flower{i}
            indice_corrente=j;
...
Xn1=[];
for i=1:k1
    Xn1=[Xn1 X1_star(i)*ones(1,cluster_flower(i))];
end
Xn2=[];
for i=1:k2
    Xn2=[Xn2 X2_star(i)*ones(1,cluster_root(i))];
end  
...
save clementina_flower_root.mat nuove_specie_totali X1_nuove_specie X2_nuove_specie M_X1 M_X2 M_T1 M_T2 M_parametri N n1 m1 n2 m2
